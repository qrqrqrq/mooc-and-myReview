...
console.log(obj.a);     // 1
console.log(obj['a']);  // 1 注意:用方括号调用对象里的属性一定要记得加''
```
...
**特殊：**其他数字都是四舍五入，但是 .5特殊，它往大了取

```javascript
console.log(Math.round(3.4));	// 3
console.log(Math.round(3.5));	// 4	4 > 3 故取4
console.log(Math.round(-1.5));	// -1   -1>-2 故取-1
...
<script>
        ...
        var td = new Date('2022-6-7 00:00:00');
    	//为了避免刷新时候有空白产生,所以线执行一次函数再执行计时器
        down()
        // 定时器
